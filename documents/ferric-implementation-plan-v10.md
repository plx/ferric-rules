...
26. [Appendix J: Review Feedback Addressed (v10)](#appendix-j-review-feedback-addressed-v10)
...
## Appendix J: Review Feedback Addressed (v10)

This section documents feedback addressed in the v10 revision:

| # | Feedback Item | Section(s) Addressing It | Summary of Change |
|---|---------------|--------------------------|-------------------|
| 1 | Plan must be standalone (remove cross-version placeholders) | 10.2, 12, 13, Appendix A | Replaced "see previous version" placeholders with concrete scope, commands, testing gates, and dropped-feature list. |
| 2 | Retraction can double-cascade on overlapping affected tokens | 5.5.1, 6.8 | Added `TokenStore::retraction_roots` and updated retraction flow to remove each affected subtree exactly once. |
| 3 | Retraction computes new activations but dropped them | 6.8 | Added `RetractionDelta` contract and explicit enqueue path for activations produced during negative updates. |
| 4 | Cleanup invariant stronger than implementation | 6.8, 7.2-7.4 | Added centralized `dispatch_token_retracted` pattern and explicit callback expectations for all token-storing memories. |
| 5 | Pattern error code contract mismatched validator behavior | 7.7 | Added explicit `E0004` variable-scope validation and `E0005` usage for unsupported forall nesting/shape. |
| 6 | Conflict strategy enum mismatch | 6.6.1, 9.1 | Unified v1 strategy set to Depth/Breadth/LEX/MEA and marked others deferred. |
| 7 | FFI panic policy contradictory for workspace profiles | 11.3, 11.8, 15.0 | Switched to explicit profile matrix (`dev/release/test=unwind`, `ffi-dev/ffi-release=abort`) with build commands. |
| 8 | Copy-to-buffer had zero-length buffer UB edge | 11.4.1, 11.7 | Added invalid-argument rule for `buf != NULL && buf_len == 0` and fixed sample implementation accordingly. |
| 9 | Canonical FFI thread-check order not reflected in code sample | 11.2, 11.7 | Updated sample `ferric_run` to shared-borrow check before mutable borrow. |
| 10 | Determinism expectations unclear | 6.6.2, 16.6 | Added explicit activation-ordering contract: total order within a run, no cross-run replay guarantee. |
| 11 | `No global state` wording and thread-affinity messaging inconsistent | 2.1, 11.2 | Clarified allowed process-level facilities and removed wording implying cross-thread use via external synchronization. |
| 12 | Avoidable debt edges: VarId overflow + index pruning | 5.4, 6.4 | Added checked variable allocation contract and eager alpha-index pruning on removal. |
...
| 10.0 | February 2026 | - | Incorporated debt-prevention and implementation-readiness review feedback (12 items; see Appendix J) |
