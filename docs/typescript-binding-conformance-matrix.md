...
| `N-03` | `EnginePool.do(..., { signal })` rejects with `AbortError` if abort wins before the pool observes callback settlement. In-flight proxy `run` operations use the same logical-run batching and out-of-band host-cancellation mechanism as `EngineHandle.run`; N-14 defines proxy admission and accepted-work effects. |
...
| `N-12` | `EnginePool.create` defaults to one Worker only when `threads` is omitted or `undefined`; every explicit count must be a safe integer in the inclusive range `1..64`. Counts are not coerced or clamped, and there is no override. An invalid count throws `RangeError` synchronously before `create` returns a Promise or constructs a Worker. |
| `N-13` | Every registered main-to-Worker request send is transactional through `postMessage` acceptance. If the send throws while the exact entry is still owned, Ferric removes that entry and its request listener, restores its in-flight unit, wakes waiters, and rejects with the exact thrown value. A prior synchronous settlement wins by entry identity. The failure is request-local: no handle/pool poisoning, termination, respawn, replay, ID or round-robin rewind, or lease release; pool dispatch continues the applicable FIFO. Pool initialization instead rejects creation and tears down unpublished Workers. Successful root-dispatch listener cleanup remains FR-NODE-006, proxy admission follows N-14, and concurrent close sharing remains FR-NODE-010. |
| `N-14` | A `do` proxy request is accepted when its final lifetime/slot/signal gate passes immediately before request-ID allocation; the signal is gated before validation and rechecked after preprocessing. Calls accepted before abort—including lease-private queued calls—retain FIFO order, their own response/send/terminal outcome, and possible state effects. If abort beats pool-observed callback settlement, the outer `do` promptly rejects with `new DOMException("The operation was aborted", "AbortError")`; every later active-proxy method returns that rejection before validation, ID/accounting/listener/queue/post. Abort does not preempt the callback or release its N-10 lease: callback settlement plus accepted-call drain releases exactly once. Callback settlement first wins over later abort. Proxy gate precedence is inactive lifetime error, active failed-slot exact error, active closed error, then active-running abort; an accepted send retains N-13 first-settlement, and an accepted run retains N-09's partial `HaltRequested` projection. |
| `N-15` | `EnginePool.create` exposes `queueCapacity`, defaulting to `1024` waiting entries per slot and accepting only nonnegative safe integers (including normalized zero). One selected slot shares that budget across queued root requests, queued lease admissions, and lease-private owner requests; dispatched work and the admitted callback/lease do not count. Work that must wait on a full selected slot rejects, without another-slot scan, wait, retry, replay, round-robin rewind, ID/lease/accounting allocation, queue mutation, or Worker post. Existing lifecycle/cancellation/validation gates win before capacity admission. An already-full rejection installs no listener; if the replaceable cooperative-listener hook for `evaluate` or proxy `run` synchronously admits nested work, Ferric rechecks every gate and current capacity before ID allocation, lets the nested work linearize first, and removes any transient listener when the outer call rejects. A finally accepted signaled root request or waiting lease reserves its structural FIFO unit before its replaceable dequeue-listener hook; hook-time abort or registration failure reclaims it, while an earlier hook-driven dispatch/admission/terminal/close outcome wins without double settlement or stale listener. Structural dequeue/removal otherwise owns capacity reclamation, with N-14's accepted-owner abort exception and N-13's no-double-reclaim send rule. Synchronous detached `metrics()` snapshots report the configured per-slot limit plus aggregate and stable per-slot queued, in-flight, and queue-full rejection counts through callback, failure, and close states. |
...
| `C-006` | Selected-slot queue overflow rejects with exported `EnginePoolQueueFullError extends FerricError`: name `EnginePoolQueueFullError`, code `FERRIC_POOL_QUEUE_FULL`, exact message `EnginePool queue is full`, and readonly `capacity`, `queued`, and `slotIndex` rejection-snapshot fields. The scheduler-local class is not reconstructed through the Worker `ERROR_REGISTRY`. | Fill a deterministic slot, assert class/prototype/name/code/message/fields and Promise-only rejection across root, lease-admission, and proxy paths; inspect the public entrypoint and registry boundary. | EnginePool bounded backpressure + N-15 | FR-NODE-011 | PASS |
...
| `D-010` | Once Worker construction succeeds, every rejected `EngineHandle.create()` clears its initialization bookkeeping and registered listeners, invokes and awaits `terminate()` exactly once before rejecting, and preserves the primary failure by identity. A termination failure is attached when cleanup can define or redefine an own writable/configurable cause property on the primary `Error`; attachment is best-effort when that descriptor update is rejected and for non-`Error` primaries because identity takes precedence. Successful initialization transfers the live Worker unchanged. Pre-Worker validation and Worker-constructor throws own no Worker; ordinary request-send rollback is D-011 and concurrent public close barriers remain FR-NODE-010. | Real invalid-source subprocess exits naturally with active resources back at baseline; injected source/snapshot protocol failures, synchronous init send failure, delayed/rejected termination, replaceable/immutable/locked-cause primary failures, duplicate terminal signals, constructor/pre-spawn failures, and successful construction controls. | EngineHandle failed-create ownership | FR-NODE-004 | PASS |
| `D-011` | An ordinary `EngineHandle` request whose `postMessage` throws applies N-13: it rejects its Promise with the exact thrown value after exact-entry pending rollback and, for `run`, request-owned abort-listener removal. A prior synchronous settlement wins. The handle retains its Worker, shared listeners, and forward-only request IDs, and a later valid request can succeed. Handle initialization remains D-010. | Force an uncloneable ordinary request and injected ordinary/run send throws; assert Promise-only exact-identity rejection, pending and abort-listener baselines before observation, unchanged Worker listeners/termination state, no ID reuse, successful later request, and response/error/exit-before-throw first-settlement controls. | Worker protocol + N-13 | FR-NODE-008 | PASS |
...
| `E-006` | `do(..., {signal})` rejects when abort wins before callback settlement (per `N-03`); E-016 defines proxy admission and accepted-work effects. | Abort during callback/proxy operations; expect rejection. | Cancellation semantics + N-03 | TSB-004 | PASS |
...
| `E-014` | Pool creation applies N-12 before any Worker side effect: invalid explicit thread counts throw a synchronous `RangeError`, while `1`, `64`, omitted, and `undefined` are accepted. | Direct-call invalid-value table for `NaN`, both infinities, zero, negatives, fractions, safe integers above 64, and unsafe integers; assert no Promise is returned and a Worker-constructor seam records zero calls. Exercise accepted boundaries with deterministic Workers and a timeout-guarded invalid-count subprocess that exits naturally. | EnginePool construction + N-12 | FR-NODE-007 | PASS |
| `E-015` | Every EnginePool initialization, immediate/queued root, and immediate/queued lease send applies N-13. Owned rollback restores pending/in-flight/listener/waiter and lease-call accounting, preserves exact error identity and pool health, and continues the correct FIFO across consecutive failures. It does not poison, replay, reschedule, release a lease, or rewind allocation history. Pool-init failure rejects creation and cleans every unpublished Worker. | Inject throws at every send site plus real uncloneable root/proxy inputs; assert exact Promise rejection, entry/counter/listener/waiter/lease baselines, correct root and lease FIFO progress through consecutive failures, later success, no terminal state/respawn/replay/ID or round-robin rewind, pool-create teardown, close completion, and response/terminal-before-throw first-settlement controls. | Worker protocol + N-13 | FR-NODE-008 | PASS |
| `E-016` | `EnginePool.do` applies N-14 at every cancellation phase. Abort closes only future proxy admission; every request accepted before abort drains with its own outcome, callback settlement still owns one lease release, and no post-abort admission can allocate or dispatch. | Real post-abort mutation reproducer; deterministic preflight, queued admission, admission-race, between-awaits, validation-race, active-native-run, callback-first, abort-first, send/fault precedence, all-proxy-method, retained-proxy, listener/lease-accounting, and seeded race tests. Assert exact `AbortError`/lifetime/terminal phase errors, accepted queued state effects and partial run result, Promise-only rejection, no new ID/accounting/listener/queue/post, release exactly once, and listener baselines. | EnginePool callback cancellation + N-03/N-09/N-10/N-11/N-13/N-14 | FR-NODE-009 | PASS |
| `E-017` | EnginePool bounded admission and metrics apply N-15 exactly: construction validates the finite per-slot limit synchronously; mixed root/lease-private traffic never retains more than the configured waiting count; overflow is prompt, typed, selected-slot-local, and FIFO-neutral; every dequeue, abortable removal, terminal cleanup, close rejection, and synchronous-send path reclaims exactly once; and snapshots remain detached and internally consistent in every lifecycle state. | Test omitted/`undefined` default, zero, valid boundaries, invalid-value/no-Worker construction, capacity+N mixed traffic, RR advancement without cross-slot rescue, guard/validation/abort/overflow precedence, no ID/accounting/queue/post or retained listener on rejection, already-full zero-listener admission, cooperative-listener hook fill/drain revalidation, structural queued-root/lease listener reservation plus abort/registration-throw/removal-retry/dispatch/admission/terminal/close reconciliation, root and owner FIFO, N-14 accepted-owner abort retention, N-13 send rollback, fault/close cleanup, aggregate/per-slot metrics, callback/failed/closed snapshots, a memory-limited public burst subprocess, and deterministic stress. | EnginePool bounded backpressure + N-15 | FR-NODE-011 | PASS |
...
2. `C-006` / `D-003` / `D-006` / `D-009` / `D-010` / `D-011` / `E-004` / `E-006` / `E-008` / `E-011` / `E-012` / `E-013` / `E-015` / `E-016` / `E-017`
3. `A-005` / `F-*`
