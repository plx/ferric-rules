# Benchmark Measurement Protocol

## Execution Commands

### Full benchmark run
```sh
cargo bench -p ferric-rules
```

### Smoke test (CI/quick validation)
```sh
cargo bench -p ferric-rules -- --test
```

### No-plot run (CI-friendly, suppresses HTML report generation)
```sh
cargo bench -p ferric-rules --bench engine_bench -- --noplot
cargo bench -p ferric-rules --bench waltz_bench -- --noplot
cargo bench -p ferric-rules --bench manners_bench -- --noplot
```

## Criterion Configuration

Criterion defaults are used unless overridden per-benchmark:

- **Warmup:** 3 seconds
- **Measurement time:** 5 seconds
- **Sample size:** 100 iterations
- **Noise threshold:** 0.01 (1%)

These provide good signal for microbenchmarks while keeping total run time under
2 minutes for the full suite.

## Environment Guidance

### Local development

1. Close unnecessary applications to reduce background CPU contention.
2. Plug in power (laptops may throttle on battery).
3. Avoid running on initial build — let the system settle after compilation.
4. Run the benchmark suite 2-3 times; trust the median, not any single run.

### CI environments

1. Pin to a specific runner type if available (avoid shared/burstable runners).
2. Use `--noplot` to skip HTML report generation (faster, no gnuplot dependency).
3. Use `--test` mode for smoke checks (verifies benchmarks compile and execute,
   does not measure performance).
4. For regression detection, compare baseline JSON files rather than raw timing
   (Criterion supports `--save-baseline` and `--baseline` flags), and run the
   threshold gate script used by CI.

## Anti-Flake Recommendations

- **Do not** interpret sub-1% changes as meaningful — they are likely noise.
- **Do** run benchmarks at least twice before reporting regressions.
- **Do** check `target/criterion/<name>/new/estimates.json` for confidence
  intervals rather than relying solely on mean values.
- **Do not** mix benchmark runs across different hardware or OS versions.
- **Do** document the hardware/environment when publishing baseline numbers.

## Baseline Capture

To save a named baseline for comparison:

```sh
cargo bench -p ferric-rules --bench engine_bench -- --save-baseline phase6-start
```

To compare against a saved baseline:

```sh
cargo bench -p ferric-rules --bench engine_bench -- --baseline phase6-start
```

## Output Artifacts

| Artifact | Location |
|----------|----------|
| HTML reports | `target/criterion/report/index.html` |
| Per-benchmark data | `target/criterion/<name>/` |
| Baseline JSON | `target/criterion/<name>/new/estimates.json` |

## CI Integration

The CI pipeline includes:
- `bench-smoke`: `cargo bench -p ferric-rules -- --test`
- `bench-thresholds`: `./scripts/bench-thresholds.sh`
- `bench-compare`: `./scripts/bench-compare.sh --quick`

The threshold job publishes `target/bench-threshold-report.json` and
`target/bench-threshold-report.md` artifacts, plus selected Criterion estimate
JSON files. See `docs/benchmark-policy.md` for threshold values and policy.

## Comparative Benchmarks (ferric vs. CLIPS)

The comparative benchmark suite measures ferric against stock CLIPS on identical
workloads using the **hyperfine subtraction method**.

### Phases Measured

Three progressively-more-complete commands are timed per engine per workload:

| Phase | Stdin to engine | Measures |
|-------|----------------|----------|
| A | `(exit)` | Launch only |
| B | `(load "w.clp") (exit)` | Launch + Load |
| C | `(load "w.clp") (reset) (run) (exit)` | Launch + Load + Eval |

Derived phase times: `load ≈ median(B) − median(A)`, `eval ≈ median(C) − median(B)`.

### Workloads

Generated by `ferric-rules-bench-gen` using the same simplified waltz/manners logic as
the Criterion benchmarks.  All `.clp` files use only syntax both engines support.

### Running Locally

```sh
# Prerequisites: hyperfine, clips, jq
./scripts/bench-compare.sh            # full run (all workloads, 10 runs)
./scripts/bench-compare.sh --quick    # CI mode (small workloads, 3 runs)
./scripts/bench-compare.sh --docker --quick    # macOS: runs inside container
```

### Output

| Artifact | Location |
|----------|----------|
| JSON report | `target/bench-compare/report.json` |
| Markdown report | `target/bench-compare/report.md` |
| Per-phase hyperfine data | `target/bench-compare/<engine>-<workload>-phase<X>.json` |
| Peak memory data | `target/bench-compare/memory.txt` |

### Interpreting Results

Ratios are `ferric / clips` — values above 1.0 mean ferric is slower or uses more
memory.  Derived phase times (load, eval) are computed by subtracting medians, so
they carry higher variance than direct measurements.  For authoritative per-phase
ferric timing, prefer the Criterion benchmark data.
