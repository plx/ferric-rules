...
fn qualified_name_parses_through_lexer() {
    ...
    let result =
        ferric_rules_parser::parse_sexprs("(SENSOR::reading)", ferric_rules_parser::FileId(0));
...
fn qualified_name_in_function_call_position() {
    ...
    let result = ferric_rules_parser::parse_sexprs(
        "(defrule test (go) => (MATH::add 1 2))",
        ferric_rules_parser::FileId(0),
    );
    assert!(result.errors.is_empty());
    let config = ferric_rules_parser::InterpreterConfig::default();
    let interp = ferric_rules_parser::interpret_constructs(&result.exprs, &config);
...
";
    ...
    let has_created = facts.iter().any(|(_, fact)| {
        if let ferric_rules_core::Fact::Ordered(of) = fact {
