...
pub fn intern(engine: &mut Engine, name: &str) -> ferric_rules_core::Symbol {
    engine
        .symbol_table
...
pub fn find_facts_by_relation(engine: &Engine, relation: &str) -> Vec<ferric_rules_core::FactId> {
    ...
    engine
        ...
        .unwrap()
        .filter_map(|(fid, fact)| {
            if let ferric_rules_core::Fact::Ordered(ordered) = fact {
...
pub fn find_template_facts(
    engine: &Engine,
    template_name: &str,
) -> Vec<Vec<(String, ferric_rules_core::Value)>> {
    ...
    engine
        ...
        .filter_map(|(_fid, fact)| {
            if let ferric_rules_core::Fact::Template(tf) = fact {
                if tf.template_id == *template_id {
                    let mut slots: Vec<(String, ferric_rules_core::Value)> = Vec::new();
