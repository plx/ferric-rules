...
#[test]
fn nonnumeric_rejection_stops_at_the_first_bad_operand() {
    // CLIPS 6.30 checks each numeric argument before evaluating the next one.
    // No later operand or RHS action should execute after a type error.
    let arguments = [
        ("(mark 1 wrong) (mark 2 4) (mark 3 8)", 1),
        ("(mark 1 4) (mark 2 \"wrong\") (mark 3 8)", 12),
        ("(mark 1 4) (mark 2 8) (mark 3 (create$ 1 2))", 123),
    ];
    for function in ["min", "max"] {
        for (arguments, trace) in arguments {
            let source = format!(
                "(defglobal ?*trace* = 0)
            ...
            assert!(matches!(
                ...
                Some(Value::Integer(actual)) if *actual == trace
            ));
            // Retain Ferric's existing ordinary assertion-error transport.
