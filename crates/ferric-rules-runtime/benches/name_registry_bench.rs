...
use ferric_rules_runtime::functions::{FunctionEnv, GenericRegistry, GlobalStore, UserFunction};
use ferric_rules_runtime::{Engine, EngineConfig, ModuleId, ModuleRegistry};
    ...
    for &module in modules {
        for (idx, name) in names.iter().enumerate() {
            store.set(
                module,
                name,
                ferric_rules_runtime::Value::Integer(idx as i64),
            );
        }
    }
    store
    ...
    c.bench_function("global_store_lookup_cycle", |b| {
        b.iter(|| {
            ...
            for _ in 0..8 {
                for &module in &modules {
                    for name in &names {
                        if let Some(ferric_rules_runtime::Value::Integer(value)) =
