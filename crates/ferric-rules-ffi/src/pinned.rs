...
fn build_options_from_ffi(
    ...
    {
        ferric_rules_runtime::EngineConfig::default()
    } else {
        ferric_rules_runtime::EngineConfig::try_from(&options.engine).map_err(|m| {
            set_global_error(m);
            FerricError::InvalidArgument
        })?
    };
...
mod option_conversion_tests {
    ...
    fn zero_initialized_pinned_options_use_engine_defaults() {
        ...
        let expected = ferric_rules_runtime::EngineConfig::default();
        assert_eq!(
    ...
    fn explicit_ascii_pinned_options_remain_ascii() {
        ...
        let expected = ferric_rules_runtime::EngineConfig::try_from(&options.engine).unwrap();
