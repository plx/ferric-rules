...
//! ```sh
//! cargo test -p ferric-rules-ffi build_matrix -- --ignored
//! ```
...
/// since `ferric-rules-ffi` lives at `crates/ferric-rules-ffi`).
...
fn cargo_build(profile: &str) -> std::process::Output {
    Command::new("cargo")
        .args(["build", "-p", "ferric-rules-ffi", "--profile", profile])
...
#[cfg(target_os = "macos")]
const CDYLIB_NAME: &str = "libferric_rules_ffi.dylib";
#[cfg(target_os = "linux")]
const CDYLIB_NAME: &str = "libferric_rules_ffi.so";
#[cfg(target_os = "windows")]
const CDYLIB_NAME: &str = "ferric_rules_ffi.dll";

#[cfg(any(target_os = "macos", target_os = "linux"))]
const STATICLIB_NAME: &str = "libferric_rules_ffi.a";
#[cfg(target_os = "windows")]
const STATICLIB_NAME: &str = "ferric_rules_ffi.lib";

// ---------------------------------------------------------------------------
// Build subprocess tests (slow — marked #[ignore])
// ---------------------------------------------------------------------------

/// Verify that `cargo build -p ferric-rules-ffi --profile ffi-dev` exits successfully.
#[test]
...
/// Verify that `cargo build -p ferric-rules-ffi --profile ffi-release` exits successfully.
#[test]
#[ignore = "invokes cargo build as a subprocess; slow, intended for CI"]
