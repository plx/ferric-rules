...
//!
//! [`PinnedEngine`] owns one dedicated worker thread and one [`ferric_rules_runtime::Engine`].
...
pub use engine::{PinnedEngine, PreDispatchCancelToken, QueueWait};
pub use error::PinnedError;
pub use options::PinnedEngineOptions;

pub use ferric_rules_runtime::{
    EngineConfig, FiredRule, HaltReason, LoadResult, RunLimit, RunResult,
};
